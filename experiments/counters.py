...
from experiments.models import Count
#import redis
#from redis.sentinel import Sentinel
#from redis.exceptions import ConnectionError, ResponseError


#COUNTER_CACHE_KEY = 'experiments:participants:%s:%s'
...
#    def old_increment(self, key, participant_identifier, count=1):
#        if count == 0:
#            return
#            cache_key = COUNTER_CACHE_KEY % key
#            freq_cache_key = COUNTER_FREQ_CACHE_KEY % key
...
#    def old_clear(self, key, participant_identifier):
#        try:
#            # Remove the direct entry
#            cache_key = COUNTER_CACHE_KEY % key
...
#    def old_get_frequency(self, key, participant_identifier):
#        try:
#            cache_key = COUNTER_CACHE_KEY % key
...
#    def old_get_frequencies(self, key):
#        try:
#            freq_cache_key = COUNTER_FREQ_CACHE_KEY % key
...
#            return tuple()
    ...
    def reset(self, key):
        try:
            cache_key = COUNTER_CACHE_KEY % key
            Count.delete(cache_key)
            freq_cache_key = COUNTER_FREQ_CACHE_KEY % key
            Count.delete(freq_cache_key)
            return True
        except :
            # Handle Redis failures gracefully
            return False
        
#    def old_reset(self, key):
#        try:
#            cache_key = COUNTER_CACHE_KEY % key
...
#    def old_reset_pattern(self, pattern_key):
#        #similar to above, but can pass pattern as arg instead
#        try:
#            cache_key = COUNTER_CACHE_KEY % pattern_key
